...
namespace Design
{
    public class QGasCalculation : CalcFormula
    {
        ...
        public bool CanCalc(object[][] values) 
        {
            try
            {
                var o = values.FirstOrDefault(d => (string)d[0] == "P шлейфа");
                if (o == null || (decimal)o[1] < 0)
                {
                    return false;
                }
                o = values.FirstOrDefault(d => (string)d[0] == "T буферное");
                if (o == null || (decimal)o[1] < 0)
                {
                    return false;
                }
                o = values.FirstOrDefault(d => (string)d[0] == "P буферное");
                if (o == null || (decimal)o[1] == 0)
                {
                    return false;
                }
